...
import { analytics, demoOrders, formatMoney } from "@/lib/demo-data";
...
type WalletEntry = { id: string; kind: "TOPUP" | "ADMIN_PAYMENT" | string; beneficiaryName: string; senderName: string; assignedToName: string; assignedToRole: string; amount: number; note: string; createdAt: string; searchDate: string };
...
const isImageSource = (value?: string) => Boolean(value && (value.startsWith("data:") || value.startsWith("blob:") || value.startsWith("http://") || value.startsWith("https://") || value.startsWith("/")));
...
export function AdminDashboard() {
  ...
  const [productForm, setProductForm] = useState({ slug: "", name: "", brand: "", description: "", category: "Mercado", weight: "", unit: "lb", cost: "", price: "", stock: "" });
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/admin/wallet-entries").then((response) => response.json()),
    ]).then(([dbProviders, dbProducts, dbOrders, dbWasteRecords, dbWalletEntries]) => {
      if (!active) return;
      ...
      setWalletEntries(dbWalletEntries);
  ...
  const filteredWasteRecords = useMemo(() => {
    const q = wasteHistorySearch.trim().toLowerCase();
    if (!q) return wasteRecords;
    return wasteRecords.filter((record) => [record.createdAt, record.productName, record.responsible, record.payer, record.cause, record.id].join(" ").toLowerCase().includes(q));
  }, [wasteRecords, wasteHistorySearch]);
  ...
  const filteredWalletEntries = useMemo(() => {
    ...
    if (!q) return walletEntries;
    return walletEntries.filter((entry) => [entry.createdAt, entry.searchDate, entry.beneficiaryName, entry.senderName, entry.assignedToName, entry.assignedToRole, entry.note, entry.id].join(" ").toLowerCase().includes(q));
  }, [walletEntries, walletSearch]);
  const walletTopups = useMemo(() => filteredWalletEntries.filter((entry) => entry.kind === "TOPUP"), [filteredWalletEntries]);
  const walletAdminAssignments = useMemo(() => filteredWalletEntries.filter((entry) => entry.kind === "ADMIN_PAYMENT"), [filteredWalletEntries]);

  const providerReportRows = useMemo(() => {
    const rows = providerList.map((provider) => {
      const productsForProvider = productList.filter((product) => product.provider === provider.name);
      const stockTotal = productsForProvider.reduce((sum, product) => sum + Number(product.stock || 0), 0);
      const inventoryCost = productsForProvider.reduce((sum, product) => sum + Number(product.stock || 0) * Number(product.cost || 0), 0);
      const inventorySaleValue = productsForProvider.reduce((sum, product) => sum + Number(product.stock || 0) * Number(product.price || 0), 0);
      const potentialMargin = inventorySaleValue - inventoryCost;
      const lowStockCount = productsForProvider.filter((product) => Number(product.stock || 0) <= 5).length;
      const wasteCount = wasteRecords.filter((record) => productsForProvider.some((product) => product.id === record.productId || product.name === record.productName)).length;
      return { provider, productsForProvider, stockTotal, inventoryCost, inventorySaleValue, potentialMargin, lowStockCount, wasteCount };
    });
    return rows.sort((a, b) => b.inventorySaleValue - a.inventorySaleValue);
  }, [providerList, productList, wasteRecords]);
  const maxProviderSaleValue = Math.max(1, ...providerReportRows.map((row) => row.inventorySaleValue));
  const providerReportTotals = useMemo(() => ({
    providers: providerReportRows.length,
    products: providerReportRows.reduce((sum, row) => sum + row.productsForProvider.length, 0),
    stock: providerReportRows.reduce((sum, row) => sum + row.stockTotal, 0),
    saleValue: providerReportRows.reduce((sum, row) => sum + row.inventorySaleValue, 0),
    margin: providerReportRows.reduce((sum, row) => sum + row.potentialMargin, 0),
    lowStock: providerReportRows.reduce((sum, row) => sum + row.lowStockCount, 0),
    waste: providerReportRows.reduce((sum, row) => sum + row.wasteCount, 0),
  }), [providerReportRows]);


  const saveWalletEntry = async (kind: "TOPUP" | "ADMIN_PAYMENT") => {
    const payload = kind === "TOPUP"
      ? { kind, ...walletTopupForm, amount: Number(walletTopupForm.amount || 0) }
      : { kind, ...walletAssignForm, amount: Number(walletAssignForm.amount || 0) };
    const response = await fetch("/api/admin/wallet-entries", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    if (!response.ok) { alert("No se pudo guardar el movimiento de billetera."); return; }
    const entry: WalletEntry = await response.json();
    setWalletEntries([entry, ...walletEntries]);
    if (kind === "TOPUP") setWalletTopupForm({ beneficiaryName: "", senderName: "", amount: "", note: "" });
    else { setWalletAssignForm({ assignedToName: "", assignedToRole: "Trabajador", amount: "", note: "" }); setShowWalletAssignForm(false); }
  };
  ...
  const resetProductForm = () => {
    ...
    setProductForm({ slug: "", name: "", brand: "", description: "", category: "Mercado", weight: "", unit: "lb", cost: "", price: "", stock: "" });
  ...
  const addProduct = async () => {
    ...
    const productPayload = {
      ...
      description: productForm.description || "Producto disponible en DREX Market Cuba.",
