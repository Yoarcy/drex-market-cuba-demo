...
export function AdminDashboard() {
  ...
  const saveWalletEntry = async (kind: "TOPUP" | "ADMIN_PAYMENT") => {
    ...
    else setWalletAssignForm({ assignedToName: "", assignedToRole: "Trabajador", amount: "", note: "" });
  };
...
{section === "Billeteras" && <section className="space-y-6"><section className="demo-card p-6"><p className="text-sm font-black uppercase tracking-[0.18em] text-emerald-700">Billeteras administrativas</p><h2 className="mt-2 text-2xl font-black text-slate-950">Saldo DREX y pagos internos</h2><p className="mt-1 text-sm font-semibold text-slate-600">Aquí se registran las recargas a beneficiarios y las billeteras/pagos asignados por el administrador. Todo queda en historial.</p><div className="mt-5 grid gap-4 md:grid-cols-3"><article className="rounded-3xl bg-emerald-50 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-700">Recargas a beneficiarios</p><p className="mt-2 text-3xl font-black text-emerald-900">{walletTopups.length}</p></article><article className="rounded-3xl bg-sky-50 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-sky-700">Asignadas por admin</p><p className="mt-2 text-3xl font-black text-sky-900">{walletAdminAssignments.length}</p></article><article className="rounded-3xl bg-amber-50 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-amber-700">Total visible</p><p className="mt-2 text-3xl font-black text-amber-900">{formatMoney(filteredWalletEntries.reduce((sum, item)=>sum + item.amount, 0))}</p></article></div></section><section className="grid gap-5 xl:grid-cols-2"><div className="demo-card p-6"><h3 className="text-xl font-black text-slate-950">Asignar dinero a beneficiario</h3><p className="mt-1 text-sm font-semibold text-slate-600">Para cuando alguien desde fuera pone dinero en la billetera de un beneficiario.</p><div className="mt-4 grid gap-3"><Field label="Nombre del beneficiario" placeholder="Ej: Mariela Pérez" value={walletTopupForm.beneficiaryName} onChange={(v)=>setWalletTopupForm({...walletTopupForm,beneficiaryName:v})}/><Field label="Quién puso el dinero" placeholder="Ej: Yordan desde España" value={walletTopupForm.senderName} onChange={(v)=>setWalletTopupForm({...walletTopupForm,senderName:v})}/><Field label="Cantidad" type="number" value={walletTopupForm.amount} onChange={(v)=>setWalletTopupForm({...walletTopupForm,amount:v})}/><Field label="Nota" placeholder="Referencia, teléfono o detalle opcional" value={walletTopupForm.note} onChange={(v)=>setWalletTopupForm({...walletTopupForm,note:v})}/><button type="button" onClick={()=>saveWalletEntry("TOPUP")} className="rounded-full bg-emerald-600 px-5 py-3 text-xs font-black text-white shadow-lg shadow-emerald-100">Guardar recarga de billetera</button></div></div><div className="demo-card p-6"><h3 className="text-xl font-black text-slate-950">Asignar billetera como forma de pago</h3><p className="mt-1 text-sm font-semibold text-slate-600">Para trabajadores, mensajeros, proveedores u otra persona que el admin decida pagar/controlar.</p><div className="mt-4 grid gap-3"><Field label="Nombre de la persona" placeholder="Ej: Carlos mensajero" value={walletAssignForm.assignedToName} onChange={(v)=>setWalletAssignForm({...walletAssignForm,assignedToName:v})}/><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Tipo / rol</span><select value={walletAssignForm.assignedToRole} onChange={(e)=>setWalletAssignForm({...walletAssignForm,assignedToRole:e.target.value})}><option>Trabajador</option><option>Mensajero</option><option>Proveedor</option><option>Administrador</option><option>Otra persona</option></select></label><Field label="Cantidad asignada" type="number" value={walletAssignForm.amount} onChange={(v)=>setWalletAssignForm({...walletAssignForm,amount:v})}/><Field label="Motivo" placeholder="Ej: pago reparto, adelanto, comisión" value={walletAssignForm.note} onChange={(v)=>setWalletAssignForm({...walletAssignForm,note:v})}/><button type="button" onClick={()=>saveWalletEntry("ADMIN_PAYMENT")} className="rounded-full bg-slate-950 px-5 py-3 text-xs font-black text-white shadow-lg shadow-slate-200">Guardar asignación admin</button></div></div></section><section className="demo-card p-6"><div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><h3 className="text-2xl font-black text-slate-950">Buscador de historial</h3><p className="mt-1 text-sm font-semibold text-slate-600">Busca por fecha, beneficiario, persona que puso dinero, trabajador, rol o nota.</p></div><input className="md:max-w-sm" placeholder="Ej: 2026-06-29, Mariela, mensajero..." value={walletSearch} onChange={(e)=>setWalletSearch(e.target.value)}/></div></section><section className="grid gap-5 xl:grid-cols-2"><div className="demo-card p-6"><h3 className="text-xl font-black text-emerald-800">Historial: billeteras con dinero</h3><p className="mt-1 text-sm font-semibold text-slate-600">Beneficiario + quién cargó + fecha + cantidad.</p><div className="mt-4 grid gap-3">{walletTopups.length ? walletTopups.map((entry)=><article key={entry.id} className="rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black text-emerald-700">{entry.id}</p><h4 className="mt-1 font-black text-slate-950">{entry.beneficiaryName}</h4><p className="text-sm font-semibold text-slate-600">Puso el dinero: {entry.senderName}</p><p className="text-xs font-bold text-slate-500">Fecha: {entry.createdAt}</p>{entry.note && <p className="mt-1 text-xs font-bold text-slate-500">Nota: {entry.note}</p>}</div><p className="rounded-full bg-white px-3 py-1 text-sm font-black text-emerald-700">{formatMoney(entry.amount)}</p></div></article>) : <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">No hay recargas que coincidan.</p>}</div></div><div className="demo-card p-6"><h3 className="text-xl font-black text-sky-800">Historial: billeteras asignadas por admin</h3><p className="mt-1 text-sm font-semibold text-slate-600">Pagos/control interno para trabajadores, mensajeros, proveedores u otros.</p><div className="mt-4 grid gap-3">{walletAdminAssignments.length ? walletAdminAssignments.map((entry)=><article key={entry.id} className="rounded-2xl border border-sky-100 bg-sky-50/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black text-sky-700">{entry.id}</p><h4 className="mt-1 font-black text-slate-950">{entry.assignedToName}</h4><p className="text-sm font-semibold text-slate-600">Rol: {entry.assignedToRole}</p><p className="text-xs font-bold text-slate-500">Fecha: {entry.createdAt}</p>{entry.note && <p className="mt-1 text-xs font-bold text-slate-500">Motivo: {entry.note}</p>}</div><p className="rounded-full bg-white px-3 py-1 text-sm font-black text-sky-700">{formatMoney(entry.amount)}</p></div></article>) : <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">No hay asignaciones que coincidan.</p>}</div></div></section></section>}
