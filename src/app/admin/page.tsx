...
{section === "Productos" && <><section className="demo-card p-6"><button onClick={() => setShowProductForm(!showProductForm)} className="flex w-full items-center justify-between text-left"><span><span className="text-2xl font-black">Agregar nuevo producto</span><span className="block text-sm text-slate-600">Formulario contraído. ID automático, imagen real y peso con unidad.</span></span><span className="rounded-full bg-emerald-50 px-4 py-2 text-sm font-black text-emerald-700">{showProductForm ? "Ocultar" : "Desplegar"}</span></button>{showProductForm && <div className="mt-5"><div className="grid gap-6 lg:grid-cols-[1fr_240px]"><div className="grid gap-4"><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Proveedor</span><select value={selectedProvider} onChange={(e)=>setSelectedProvider(e.target.value)}>{providerList.map((p)=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><Field label="Municipio automático" readOnly value={activeProvider ? `${activeProvider.municipality}, ${activeProvider.province}` : ""}/><Field label="ID automático" readOnly value={productAutoId}/></div><label className="flex min-h-56 cursor-pointer flex-col items-center justify-center rounded-[1.75rem] border-2 border-dashed border-slate-300 bg-slate-50 p-4 text-center"><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Imagen</span>{productImagePreview ? <img src={productImagePreview} alt="Preview" className="mt-3 h-36 w-36 rounded-3xl object-contain bg-white p-2"/> : <span className="mt-3 flex h-28 w-28 items-center justify-center rounded-3xl bg-white text-4xl">+</span>}<span className="mt-3 max-w-44 truncate text-xs font-semibold text-slate-500">{productImageName || "Subir archivo .jpg"}</span><input className="hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={(e)=>onImage(e.target.files?.[0])}/></label></div><div className="mt-5 grid gap-4"><Field label="Nombre identificativo" value={productForm.slug} onChange={(v)=>setProductForm({...productForm,slug:v})}/><Field label="Nombre comercial" value={productForm.name} onChange={(v)=>setProductForm({...productForm,name:v})}/><Field label="Marca" value={productForm.brand} onChange={(v)=>setProductForm({...productForm,brand:v})}/><div className="grid gap-4 md:grid-cols-[1fr_180px]"><Field label="Peso" type="number" value={productForm.weight} onChange={(v)=>setProductForm({...productForm,weight:v})}/><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">Unidad</span><select value={productForm.unit} onChange={(e)=>setProductForm({...productForm,unit:e.target.value})}><option>lb</option><option>g</option><option>kg</option></select></label></div><Field label="Stock inicial" type="number" value={productForm.stock} onChange={(v)=>setProductForm({...productForm,stock:v})}/><div className="grid gap-4 md:grid-cols-2"><Field label="Precio de compra" type="number" value={productForm.cost} onChange={(v)=>setProductForm({...productForm,cost:v})}/><Field label="Precio de venta" type="number" value={productForm.price} onChange={(v)=>setProductForm({...productForm,price:v})}/></div><button onClick={addProduct} className="btn-primary">Guardar producto demo</button></div></div>}</section><section className="demo-card p-6"><h2 className="text-2xl font-black">Lista de productos</h2><p className="mt-1 text-sm font-semibold text-slate-600">Solo se muestra ID y nombre identificativo. Pincha un producto para ver detalles y agregar stock.</p><div className="mt-5 grid gap-3">{productList.map((p)=><article key={p.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><button onClick={()=>setViewProductId(viewProductId === p.id ? null : p.id)} className="flex w-full flex-col gap-2 text-left md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-black text-emerald-700">{p.id}</p><h3 className="text-lg font-black text-slate-950">{p.slug}</h3></div><span className="rounded-full bg-white px-4 py-2 text-xs font-black text-slate-600">{viewProductId === p.id ? "Ocultar datos" : "Ver datos"}</span></button>{viewProductId === p.id && <div className="mt-4 grid gap-4 rounded-2xl bg-white p-4 md:grid-cols-[140px_1fr]"><div className="flex items-center justify-center rounded-2xl bg-slate-50 p-3">{p.image?.startsWith?.("blob:") ? <img src={p.image} alt={p.name} className="h-28 w-28 rounded-xl object-contain"/> : <span className="text-5xl">{p.image}</span>}</div><div className="grid gap-2 text-sm font-semibold text-slate-700 md:grid-cols-2"><p>Nombre: {p.name}</p><p>Proveedor: {p.provider}</p><p>Marca: {p.brand}</p><p>Peso: {p.weight}</p><p>Stock actual: {p.stock}</p><p>Imagen: {p.imageFile}</p><p>Compra: {formatMoney(p.cost)}</p><p>Venta: {formatMoney(p.price)}</p><div className="mt-2 flex gap-2 md:col-span-2"><input className="max-w-40" type="number" placeholder="Agregar stock" value={stockToAdd} onChange={(e)=>setStockToAdd(e.target.value)}/><button onClick={()=>addStock(p.id)} className="rounded-full bg-emerald-600 px-4 py-2 text-xs font-black text-white">Sumar stock</button></div></div></div>}</article>)}</div></section></>}
