...
{section === "Pedidos" && <section className="demo-card p-6"><h2 className="text-2xl font-black">Pedidos</h2><p className="mt-1 text-sm font-semibold text-slate-600">Control operativo demo: pago, entrega, peso y proveedor. Sin ventas reales.</p><div className="mt-5 grid gap-4">{orderList.map((o, index)=><article key={o.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-black text-emerald-700">{o.id}</p><h3 className="text-lg font-black text-slate-950">{o.beneficiary}</h3><p className="text-sm font-semibold text-slate-600">Cliente: {o.customer}</p><p className="text-sm font-semibold text-slate-600">Municipio: Bauta, Artemisa</p></div><span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-black text-sky-700">{o.status}</span></div><div className="mt-4 grid gap-2 text-sm font-semibold text-slate-700 md:grid-cols-2"><p>Productos: {index === 0 ? "Combo Familiar x1 + Kit Aseo x2" : index === 1 ? "Combo Desayuno x1 + Paquete Limpieza x1" : "Producto demo x1"}</p><p>Peso total reparto: {index === 0 ? "9 lb" : index === 1 ? "4 lb" : "2 lb"}</p><p>Total pagado: {formatMoney(o.total)}</p><p>Método/pago: {o.payment}</p><p>Proveedor: {index % 2 === 0 ? "Proveedor Bauta Alimentos" : "Proveedor Bauta Aseo"}</p><p>Repartidor: {o.courier}</p><p>Fecha pedido: 25/06/2026</p><p>Entrega estimada: 24-48h demo</p></div><div className="mt-4 flex flex-wrap items-center gap-2"><span className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Cambiar estado</span><select className="max-w-56" value={o.status} onChange={(e)=>changeOrderStatus(o.id, e.target.value)}><option>Recibido</option><option>Pago confirmado</option><option>Preparando</option><option>En reparto</option><option>Entregado</option><option>Cancelado</option></select><button className="rounded-full bg-white px-4 py-2 text-xs font-black text-slate-600">Ver detalles</button></div></article>)}</div></section>}
