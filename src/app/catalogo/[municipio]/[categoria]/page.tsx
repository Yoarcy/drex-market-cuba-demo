...
import { catalogCategories, catalogMunicipalities, getCategoriesForMunicipality, getCategoryBySlug, getMunicipalityBySlug, getProductsForMunicipalityCategory } from "@/lib/catalog";

export function generateStaticParams() {
  return catalogMunicipalities.flatMap((municipality) =>
    catalogCategories.map((category) => ({ municipio: municipality.slug, categoria: category.slug })),
  );
}
...
export default async function CategoryCatalogPage({ params }: { params: Promise<{ municipio: string; categoria: string }> }) {
  ...
  const routeProducts = getProductsForMunicipalityCategory(municipio, categoria);
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-slate-600">Esta ruta carga únicamente productos demo de {category.name} para {municipality.name}.</p>
        </div>
